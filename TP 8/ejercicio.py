...
if archivo:
    ...
    if opcion_elegida == "Todas":
        ...
        st.title(f"Datos de {opcion_elegida}")
    ...
    for producto, dato_producto_agrupado in productos_agrupados:
        ...
        with st.container(border=True):
            ...
            margen_promedio = ((dato_producto_agrupado["Ingreso_total"].sum() - dato_producto_agrupado["Costo_total"].sum()) / max(dato_producto_agrupado["Ingreso_total"].sum(), 1)) * 100
            ...
            precio_anterior = max(dato_producto_agrupado["Ingreso_total"].iloc[-2] / max(dato_producto_agrupado["Unidades_vendidas"].iloc[-2], 1), 1) 
            margen_anterior = max(((dato_producto_agrupado["Ingreso_total"].iloc[-2] - dato_producto_agrupado["Costo_total"].iloc[-2]) / max(dato_producto_agrupado["Ingreso_total"].iloc[-2], 1)) * 100, 1) 
            unidades_anteriores = max(dato_producto_agrupado["Unidades_vendidas"].iloc[-2], 1) 

            variacion_precio = ((precio_promedio - precio_anterior) / max(precio_anterior, 1)) 
            variacion_margen = ((margen_promedio - margen_anterior) / max(margen_anterior, 1))  
            variacion_unidades = ((total_unidades_vendidas - unidades_anteriores) / max(unidades_anteriores, 1))
